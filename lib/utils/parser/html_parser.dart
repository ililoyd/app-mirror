        ...
        switch (node.localName) {
          ...
          case "span":
            ...
            if (attrList.containsKey("style")){
              String value = attrList["style"];

              RegExp regExp = new RegExp(r"color: #([0-9a-fA-F]{6})");
              if(regExp.hasMatch(value)){     
                Color hexToColor(String code) {
                  return new Color(int.parse(code, radix: 16) + 0xFF000000);
                }

                Color colorSpan = hexToColor(regExp.firstMatch(value).group(1)); 
                childStyle =
                 childStyle.merge(TextStyle(color: colorSpan));
              }
          ...
          default:
            ...
            BlockText blockText = BlockText(
              margin: EdgeInsets.only(
                  ...
                  bottom: 8.0,
                  left: parseContext.indentLevel * indentSize),
              padding: (parseContext.blockType == 'blockquote') ? EdgeInsets.all(8.0 ): EdgeInsets.all(2.0),
              decoration: decoration,
              child: RichText(
                textAlign: textAlign,
                text: TextSpan(
